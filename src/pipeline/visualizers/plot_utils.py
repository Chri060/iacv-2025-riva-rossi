...
def reference_frame(ax, pos, rot, colors=['r', 'g', 'b'], names=['x', 'y', 'z'], length=1, label=None, lcolor=None):
    if lcolor == None:
...
def camera(ax, camera, lcolor=None):
    reference_frame(ax, camera.position, camera.rotation, length=3, label=camera.name, lcolor=lcolor)
