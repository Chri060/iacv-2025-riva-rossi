...
class Intrinsic_Calibration(Pipe):
    def __process_params(self, params):
        try : images_path = params['images_path']
        except: raise Exception("Missing required parameter : images_path")
        try : visualization = params['visualization'] 
        except: visualization = Environment.visualization
        try : checkerboard_sizes = params['checkerboard_sizes']
        except: checkerboard_sizes = [[9,6], [9,6]]
