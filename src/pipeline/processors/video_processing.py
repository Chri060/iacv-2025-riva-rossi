...
class Synchronizer(Pipe):
    def execute(self, params):
        # Load Parameters
        try: save_path = params['save_path']
        ...
        if visualization: cv.destroyAllWindows()
    ...
    def load(self, params):
        ...
        if visualization:
            ...
            while cap1.isOpened() and cap2.isOpened():
                ...
                cv.imshow(f"Synchronization of {view1.camera.name} and {view2.camera.name}", stacked_frame)
                cv.waitKey(1)
    ...
    def __apply_shift(self, videos, output_path1, output_path2, target_fps, target_duration, start1, start2, visualization):
