        ...
        while True:
            ...
            if stabilization:
                ...
                if len(good_new) > 0 and stabilization:
                    ...
                    transformation_matrix, _ = cv.estimateAffinePartial2D(
                        good_old, good_new
                    )

                    # Warp the frame to stabilize it
                    frame = cv.warpAffine(
                        frame, transformation_matrix, (frame_width, frame_height)
                    )
            ...
            _, diff_thresholded = cv.threshold(
                difference_gray, threshold, 255, cv.THRESH_BINARY
            )
            ...
            if len(indices) != 0 and not np.any(pos is None):
                indices = indices[
                    np.linalg.norm(indices - pos, axis=1) <= tracking_radius
                ]
